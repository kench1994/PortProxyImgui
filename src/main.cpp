...
int main(int, char**)
{
    ...
    // Load Fonts
    io.Fonts->AddFontFromMemoryCompressedTTF((void*)font_compressed_data, font_compressed_size, 18.0f, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
    ...
    std::vector<std::shared_ptr<bool>> vChkList;
    for (auto idx = 0; idx < vRuleList.size(); idx++)
        vChkList.emplace_back(std::make_shared<bool>(false));

    auto fnUpdateMode = [&vRuleList, &vChkList](){
    ...
    while (!done)
    {
        ...
        if (ImGui::Begin("PortProxy", &show_main_window, window_flags))
        {
            if (ImGui::BeginTable("table_padding", 3, ImGuiTableFlags_SizingStretchProp))
            {
                ...
                for (const auto& spRule : vRuleList)
                {
                    ...
                    for (int column = 0; column < 3; column++)
                    {
                        ...
                        {
                        case 0:
                            if(ImGui::Checkbox(std::to_string(row + 1).c_str(), vChkList[row].get()))
...

            {
                ...
                ImGui::SameLine();
                ImGui::PopItemWidth();
                if(ImGui::Button("添加路由"))
                {
                    show_add_windows = true;

                }
                ImGui::SameLine();
                if(ImGui::Button("删除路由"))
                {
                    strErrOnDel.clear();
                    int i = 0;
                    std::string strErrInfo;
                    auto vRuleListCp = vRuleList;
                    for(const auto& spRule : vRuleListCp)
                    {
                        if(*vChkList[i++])
                        {
        ...
        if (show_add_windows)
        {
            ...
            ImGui::InputText("起始端口 -", downstream_port, IM_ARRAYSIZE(downstream_port), ImGuiInputTextFlags_CharsDecimal);
            ImGui::SameLine();
            ...
            if(ImGui::Button("确定"))
            {
                strErrOnAdd.clear();
                do{
                    if(!strlen(downstream_server) || !strlen(listen_port) || !strlen(downstream_port))
                    {
                        strErrOnAdd = "缺失关键参数";
                        break;
                    }
                    ...
                    int make_pair = 0;
                    if(strlen(downstream_port_end))
                        make_pair++;
                    if(strlen(listen_port_end))
                        make_pair++;
                    if(2 == make_pair)
                    {
                        listen_end = atoi(listen_port_end);
                        forward_end = atoi(downstream_port_end);
                        if(0 != (forward_end - forward_begin - listen_end + listen_begin))
                        {
                            strErrOnAdd = "转发端口组数量不一致";
                            break;
                        }
                    }
                    for(auto i = listen_begin; i <= listen_end; i++)
                    {
                        std::string strErrInfo;
                        auto spRule = std::make_shared<port_proxy::model::Rule>();
                        spRule->uInPort = i;
                        spRule->uOutPort = forward_begin++;
                        sprintf_s(spRule->szOutAddress, sizeof(spRule->szOutAddress), downstream_server);
